...
import { selfHealClick } from "../helpers/common.js";
...
export class EmiCalculatorPage {
    constructor(page) {
        ...
        // ---------------------------
        // AI Self-Healing Demo (Intentionally Broken Locators)
        // ---------------------------
        this.brokenPersonalLoanTab = this.page.locator("#personal-loan-old");
        this.brokenLoanAmount = this.page.locator("#loanamount-old");
        this.brokenInterestRate = this.page.locator(".interest-rate-old");
        this.brokenLoanTenure = this.page.locator("#loanterm-old");
        this.brokenPieChart = this.page.locator("#fake-pie-chart");
            }
    ...
    async getTotalInterest() {
        ...
        return Number(
            totalInterestText.replace(/[₹,]/g, "").trim()
        );
    }
    ...
    async getTotalAmount() {
        ...
        return Number(
            totalAmountText.replace(/[₹,]/g, "").trim()
        );
    }
    ...
    async waitForEMIToUpdate(expectedEMI) {
        await expect(this.emiAmount).toHaveText(
            expectedEMI.toLocaleString("en-IN")
        );
    }

    async navigateToPersonalLoan() {
        await selfHealClick(
            this.personalLoanTab,
            this.page.getByText("Personal Loan"),
            this.page.locator("a[href='#personal-loan']")
        );
    }
